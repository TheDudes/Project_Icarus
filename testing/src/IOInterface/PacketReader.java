...
public class PacketReader extends BufferedInputStream{
    ...
    */
    ...
    /**
    * transforms a 2 byte Array into an integer, for this method the byteArray should be formatted in a specific Format
    * this means: the second field(which displays the magnitude 0-7 from 2) is stored in the second byte via the Integer.byteValue()
    * the first field displays the magnitudes 8-15
    * the way we get those numbers is by looking which bits are 1 and which 0 and then assigning them the proper magnitude
    * this means that if the number is higher than 128 the bit for the value 32768 is set which means we add this number to the int
    * rinse and repeat for the smaller magnitudes
    *
